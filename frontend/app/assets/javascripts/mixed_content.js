...
$(function () {
      ...
      var onWrapActionChange = function (event) {
        if ($editor.somethingSelected() && $wrapWithActionSelect.val() != '') {
          ...
          $editor.replaceSelection(
            '<' + tag + attributes + '>' + $editor.getSelection() + '</' + tag + '>'
          );
  ...
  $(document).bind("loadedrecordform.aspace", function(event, object_name, subform) {
    $("textarea.mixed-content:not(.initialised):visible", subform).mixedContent();
  });
